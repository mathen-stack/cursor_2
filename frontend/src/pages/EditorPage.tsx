...
/**
 * Split comma-separated skills without dropping trailing empties.
 * That keeps controlled inputs usable while the user types "Python, ".
 */
function splitSkills(text: string): string[] {
  return text.split(",").map((s) => s.trimStart())
}
...
/** Preserve blank lines so Enter works in controlled bullet textareas. */
      ...
      <section className="space-y-4 rounded-xl border border-border/80 bg-cream/60 p-5 sm:p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <Input
              value={joinSkills(resume.skills)}
              onChange={(e) => patchResume({ skills: splitSkills(e.target.value) })}
            />
          </div>
        </div>
      ...
      <section className="space-y-4 rounded-xl border border-border/80 bg-cream/60 p-5 sm:p-6">
        ...
        <div>
          ...
          <Textarea
            ...
            onChange={(e) =>
              patchLetter({
                body_paragraphs: splitParagraphs(e.target.value),
              })
            }
          />
        </div>
